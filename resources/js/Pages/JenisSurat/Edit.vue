                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div v-for="persyaratan in persyaratans" :key="persyaratan.id"
                                class="flex items-center mb-2">
                                <input type="checkbox" :value="persyaratan.id" v-model="form.persyaratan_ids"
                                    class="form-checkbox h-5 w-5 text-blue-600 transition duration-150 ease-in-out"
                                    :checked="form.persyaratan_ids.includes(persyaratan.id)" />
...
const form = useForm({
    name: props.jenisSurat.name,
    form_id: props.jenisSurat.form_id,
    persyaratan_ids: props.jenisSurat.persyaratans ? props.jenisSurat.persyaratans.map(p => p.id) : []
});

const submit = () => {
    form.put(route('jenis-surats.update', { jenis_surat: props.jenisSurat.id }), {
        onSuccess: () => {
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: 'Jenis Surat updated successfully!',
            });
        },
        onError: (errors) => {
            Swal.fire({
