            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row justify-between mb-4">
                        <form @submit.prevent="searchUsers" class="relative w-full sm:max-w-xs mb-4 sm:mb-0">
                            <input v-model="q" type="text" placeholder="Search users..."
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr v-for="(user, index) in users.data" :key="user.uuid">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ user.nik.substring(0, 3) + '****' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ user.nokk.substring(0, 3) + '****' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <button @click="showUser(user.uuid)"
                                            class="btn btn-info bg-blue-500 p-2 rounded mr-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white"
                                                viewBox="0 0 24 24" fill="currentColor">
                                                <path
                                                    d="M12 4.5c-4.97 0-9 4.03-9 9s4.03 9 9 9 9-4.03 9-9-4.03-9-9-9zm0 16.5c-4.14 0-7.5-3.36-7.5-7.5s3.36-7.5 7.5-7.5 7.5 3.36 7.5 7.5-3.36 7.5-7.5 7.5zm-1-11h2v6h-2zm0 8h2v2h-2z" />
                                            </svg>
                                        </button>
                                        <button @click="editUser(user.uuid)"
...
const searchUsers = () => {
    router.get(route('users.index'), { q: q.value }, {
        preserveState: true,
        onSuccess: (page) => {
...
const editUser = (uuid) => {
    router.get(route('users.edit', { uuid }));
};
...
const showUser = (uuid) => {
    router.get(route('users.show', { uuid }));
};
</script>
